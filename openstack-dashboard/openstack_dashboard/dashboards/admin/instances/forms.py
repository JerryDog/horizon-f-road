...
#Add by liuchunbo
class MigrateForm(forms.SelfHandlingForm):
    current_instance = forms.CharField(label=_("Current Instance"),
                                        required=False,
    ...
    def handle(self, request, data):
        try:
            obj_id=data['instance_id']
            host=data['host']
            api.nova.server_migrate(request, obj_id, host)
            msg = _('The instance is preparing the migration '
                    'to host "%s".') % data['host']
            messages.success(request, msg)
            return True
        except Exception:
            msg = _('Failed to migrate instance to '
                    'host "%s".') % data['host']
